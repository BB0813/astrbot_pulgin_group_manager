from astrbot.api.event import filter, AstrMessageEvent from astrbot.api.star import Context, Star, register from astrbot.api import logger
@register("astrbot_plugin_group_manager", "mjy1113451", "智能群管理插件 - 支持正则表达式/关键词/白名单/黑名单验证加群申请", "v1.1.0") class GroupManager(Star): """GroupManager 插件主类"""
def __init__(self, context: Context, config: dict = None):
    super().__init__(context)

    # 延迟导入核心模块，避免循环依赖或启动时加载过重
...
async def gm_admin_add(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_admin_remove(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_admin_list(self, event: AstrMessageEvent, *args):
...
@gm.command("enable")
async def gm_enable(self, event: AstrMessageEvent, *args):
...
@gm.command("disable")
async def gm_disable(self, event: AstrMessageEvent, *args):
...
async def gm_whitelist_add(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_whitelist_remove(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_whitelist_list(self, event: AstrMessageEvent, *args):
...
async def gm_blacklist_add(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_blacklist_remove(self, event: AstrMessageEvent, user_id: str = None, *args):
...
async def gm_blacklist_list(self, event: AstrMessageEvent, *args):
...
async def gm_help(self, event: AstrMessageEvent, *args):
    """ 显示帮助信息 用法: /gm help """
